...
pub fn get_opcode_string(opcode: &OpcodeType, opcode_list: &Vec<OpcodeType>) -> String {
    match opcode {
        ...
        OpcodeType::OpAdd => "memory[inst[$A_REGISTER$]] = (constantB(inst) or 0) + (constantC(inst) or 0)".to_string(),
        OpcodeType::OpSub => "memory[inst[$A_REGISTER$]] = (constantB(inst) or 0) - (constantC(inst) or 0)".to_string(),
        OpcodeType::OpMul => "memory[inst[$A_REGISTER$]] = (constantB(inst) or 0) * (constantC(inst) or 0)".to_string(),
        OpcodeType::OpDiv => "memory[inst[$A_REGISTER$]] = (constantB(inst) or 0) / (constantC(inst) or 0)".to_string(),
        OpcodeType::OpMod => "memory[inst[$A_REGISTER$]] = (constantB(inst) or 0) % (constantC(inst) or 0)".to_string(),
